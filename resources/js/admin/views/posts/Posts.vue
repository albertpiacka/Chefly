<template>
    <div class="content-wrapper" data-aos="fade-up">
        <b-card bg-variant="dark" text-variant="white" title="Posts" style="border-radius: 1em 1em 0 0">
            ...
            <router-link to="/admin/new-post" class="text-white">
            ...
            <table class="table table-striped shadow-lg" v-show="loaded">
                ...
                <transition-group tag="tbody" name="fade">
                    <tr v-for="post in filteredItems" :key="post.id">
                        ...
                        <td>
                            <router-link class="text-muted" :to="'/admin/users/' + post.user.id">{{post.user.name}} ➜</router-link>
                        </td>
                        <td>
                            {{prettyDate(post.created_at)}}
                        </td>
                        <td>
                            <table-controls resource="post" :id="post.slug" />
                        </td>
                    </tr>
                </transition-group>    
            </table>
    ...
    export default {
        ...
        mounted () {
            ...
            this.$root.$on('delete-this', data => {
                this.data = this.data.filter(item => item.slug !== data[1])
            })    
        },
    }
</script>
